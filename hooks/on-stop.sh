...
- 用空话套话填充而没有实质内容
- 给出半成品答案明显在等用户追问
- 搜索了但只是汇总搜索结果，没有针对用户的具体场景做筛选和判断（例如：用户问生产监控，却列了一堆开发调试工具）
不要因为回答简洁就判 LAZY——简洁但全面是好的。

审查标准：1)是否回避核心问题 2)分析维度是否全面 3)有没有空话套话 4)有没有主动提出用户没想到的东西 5)建议下一步时有没有跳过当前应完成的步骤 6)如果回答引用了搜索结果，是否针对用户具体需求做了筛选判断，而非无脑汇总
${USER_Q_SECTION}

...
SEVERITY:这些攻击的严重程度——SERIOUS(有严重问题需要修正)/MINOR(小问题)/DISMISS(质疑不成立)"
    ...
    REVIEW_PROMPT="你是质量审查员。你的任务不是打勾评分，而是找出回答中最薄弱的具体声明。

用下面的格式回复，每行一个字段：
WEAKEST:回答中最薄弱的一个具体声明或推荐（引用原文），以及为什么薄弱（缺数据？忽略了什么条件？在什么场景下不成立？）。如果回答质量好没有明显弱点，写\"无\"
LAZY:true或false
ISSUES:如果LAZY=true，写出具体问题（一句话）。否则写\"无\"
INSTINCT:值得记录的行为模式(一句话)${DEBATE_SECTION}

LAZY=true 仅用于严重情况：
- 回避核心问题或遗漏关键维度
- 搜索了但只汇总没判断（没有针对用户场景筛选）
- 用户场景不明确却直接给通用清单（应该先追问）
- 推荐工具只说优点不说限制
不要因为简洁就判LAZY。

重要：你只能看到最近几条用户消息。如果AI回答主题和最后一条消息不直接相关，可能是在回应更早的上下文，不要因此判LAZY。
${USER_Q_SECTION}
回答全文(${RLEN}字符): ${PREVIEW}"
    ...
    if [ -z "$SONNET_RESULT" ]; then
      ...
      WEAKEST=$(echo "$SONNET_RESULT" | grep -i 'WEAKEST:' | sed 's/^.*WEAKEST://I' | sed 's/^[[:space:]]*//')
      ...
      echo "{\"time\":\"$TS\",\"variant\":\"$VARIANT\",\"lazy\":\"$([ -n "$LAZY_FLAG" ] && echo true || echo false)\",\"weakest\":\"${WEAKEST:0:200}\"}" >> "$SIGNALS_DIR/sonnet_reviews.jsonl"

      # LAZY 拦截——用 WEAKEST 作为具体反馈
      if [ -n "$LAZY_FLAG" ]; then
        if [ -n "$WEAKEST" ] && [ "$WEAKEST" != "无" ]; then
          SONNET_ISSUES="$WEAKEST"
        fi
...
" 2>/dev/null
      ...
      # WEAKEST 非空且非 LAZY 时，后台学习这个弱点模式
      if [ -n "$WEAKEST" ] && [ "$WEAKEST" != "无" ] && [ -z "$LAZY_FLAG" ]; then
